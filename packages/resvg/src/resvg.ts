...
export function initResvg(
	...
	if (initResvg.initialized) {
		throw new Error(
			'(@cf-wasm/resvg): Function already called. The `initResvg()` function can be used only once.',
		);
	}
	if (!input) {
...
initResvg.ensure = (): Promise<void> => {
	if (!initResvg.promise) {
		throw new Error(
			'(@cf-wasm/resvg): Function not called. Call `initResvg()` function first.',
		);
	}
	return initResvg.promise;
};
...
export class Resvg extends ResvgClass {
	constructor(svg: Uint8Array | string, options?: ResvgRenderOptions) {
		if (!initResvg.ready) {
			...
			throw new Error(
				'(@cf-wasm/resvg): Resvg is not yet initialized. Call `initResvg()` function first.',
			);
		}
		super(svg, options);
	}
