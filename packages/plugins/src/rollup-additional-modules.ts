...
export default function additionalModules(
	options: AdditionalModulesOptions = {},
): Plugin {
	...
	return {
			...
			async handler(source, importer) {
				if (matchModule(source)) {
					...
					if (resolved?.id && resolved.id !== source) {
						return {
							id: resolved.id.startsWith('file://')
								? resolved.id.slice(7)
								: resolved.id,
							external: false,
		...
		load: {
			...
			async handler(id) {
				...
				try {
					...
					throw new Error(
						`Module "${modulePath}" not found. Does the file exists?`,
						{
							cause: error,
						},
					);
				...
				if (
					target !== 'inline' &&
					(target !== 'edge-light' || moduleType === 'CompiledWasm')
				) {
		...
		async renderChunk(code, chunk) {
			...
			if (
				!chunk.moduleIds.some((id) => isModuleReference(id)) &&
				!chunk.imports.some((id) => isModuleReference(id))
			) {
				return;
			}
			...
			for (const match of code.matchAll(moduleReferenceMatchGlobalRE)) {
				...
				const prebuiltChunkRelativePath = relative(
				...
				if (assetId && (target === 'workerd' || target === 'edge-light')) {
					...
					const assetRelativePath = relative(
						dirname(chunk.fileName),
						assetFileName,
					);
		...
		async generateBundle(_, bundle) {
			...
			for (const replacement of replacements) {
				const chunkReplacements =
					replacementsByChunkName.get(replacement.chunkName) || [];
			...
			for (const [fileName, repls] of replacementsByFileName.entries()) {
				...
				for (const replacement of repls) {
					...
					updated = updated.replaceAll(
						replacement.fromImport,
						replacement.toImport,
					);

					const prebuiltChunkPath = join(
						outDir,
						replacement.prebuiltChunkFileName,
					);
