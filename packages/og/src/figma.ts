...
if (!modules.isUsable()) {
  throw new Error(
    `Modules are not initialized! Please import the main module based on runtime (i.e. '@cf-wasm/og', '@cf-wasm/og/edge-light', etc.) before importing the 'figma' submodule (i.e. '@cf-wasm/og/figma').
For example:
------------
import '@cf-wasm/og/edge-light'; // <- import main module based on runtime first
