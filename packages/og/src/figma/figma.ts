 ...
 */
export const assertValue = (value: string | undefined, errorMessage: string) => {
 ...
 */
const match = (string: string, matcher: RegExp, index = 1, defaultValue = '') => {
...
export const isComplexTemplate = (template: unknown) =>
  typeof template !== 'string' && template !== undefined && 'value' in (template as { value?: unknown });
 ...
 */
export const removeTextNodes = (svg: string) => svg.replace(/<text[^>]*>(.*?)<\/text>/g, '');
...
export const loadTextNodeFonts = async (nodeAttributes: ReturnType<typeof parseTextNode>[], loadFonts: LoadFontsFunction) =>
  (
    await Promise.all(
      nodeAttributes
        .filter(
 ...
 */
export const renderFigma = (figmaOptions: FigmaOptions, renderOptions?: RenderFigmaOptions) => {
