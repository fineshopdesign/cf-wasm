...
export function initPng(
	...
	if (initPng.initialized) {
		throw new Error(
			'(@cf-wasm/png): Function already called. The `initPng()` function can be used only once.',
		);
	}
	if (!input) {
...
initPng.sync = (
	input: SyncInitInput | { module: SyncInitInput },
): InitOutput => {
	if (initPng.initialized) {
		throw new Error(
			'(@cf-wasm/png): Function already called. The `initPng()` function can be used only once.',
...
initPng.ensure = (): Promise<InitOutput> => {
	if (!initPng.promise) {
		throw new Error(
			'(@cf-wasm/png): Function not called. Call `initPng()` function first.',
		);
	}
	return initPng.promise;
};
