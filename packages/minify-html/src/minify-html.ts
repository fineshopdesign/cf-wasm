...
async function loadWasm(
	...
	const instantiated = (await WebAssembly.instantiate(source, imports)) as
...
export function initMinifyHTML(
	input: InitInput | Promise<InitInput>,
): Promise<void> {
	if (initMinifyHTML.initialized) {
		throw new Error(
			'(@cf-wasm/minify-html): Function already called. The `initMinifyHTML()` function can be used only once.',
		);
	}
	if (!input) {
...
initMinifyHTML.ensure = (): Promise<void> => {
	if (!initMinifyHTML.promise) {
		throw new Error(
			'(@cf-wasm/minify-html): Function not called. Call `initMinifyHTML()` function first.',
		);
	}
	return initMinifyHTML.promise;
};
...
export function minify(
	...
	if (!initMinifyHTML.ready) {
		...
		throw new Error(
			'(@cf-wasm/minify-html): minify-html is not yet initialized. Call `initMinifyHTML()` function first.',
		);
	}
